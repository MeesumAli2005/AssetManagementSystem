...
async function validateAndWriteSpecValues(
  ...
  for (const sv of specValues || []) {
    if (!validSpecIds.includes(sv.category_spec_id)) {
      throw new Error(
        `category_spec_id ${sv.category_spec_id} does not belong to this asset's category`,
      );
    }
  }

  const missingRequired = categorySpecs.filter(
    (s) => s.is_required && !providedIds.includes(s.id),
  );
  ...
  await connection.query("DELETE FROM asset_spec_values WHERE asset_id = ?", [
    assetId,
  ]);
...
export async function getAllAssets(req: Request, res: Response) {
  try {
    ...
    if (search) {
      conditions.push("(a.name LIKE ? OR a.asset_tag LIKE ?)");
  ...
  try {
    ...
    if (req.user!.role !== ROLES.ADMINISTRATOR) {
      ...
      if (!hasAccess) {
        return res
          .status(403)
          .json({ message: "You do not have access to this asset" });
      }
      ...
      asset.my_assignment =
        assignmentRows.find((a) => a.is_active === 1) || null;
    }
  ...
  try {
    ...
    if (assignee_id !== undefined) {
      if (finalAssigneeId !== null) {
        ...
        finalStatus =
          existing.status === "assigned" ? "available" : existing.status;
      }
    } else {
    ...
    if (spec_values !== undefined) {
      if (!finalCategoryId) {
        throw new Error(
          "Cannot set spec_values on an asset with no category_id",
        );
      }
...
export async function getPendingAcknowledgements(req: Request, res: Response) {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error fetching pending acknowledgements" });
  }
}
...
export async function getMyAcknowledgements(req: Request, res: Response) {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error fetching your acknowledgements" });
  }
}
...
export async function getAcknowledgementById(req: Request, res: Response) {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error fetching acknowledgement" });
  }
}
...
export async function acknowledgeAssignment(req: Request, res: Response) {
  ...
  try {
    ...
    if (rows.length === 0) {
      return res.status(404).json({
        message: "No pending assignment to acknowledge for this asset",
      });
    }
    ...
    return res
      .status(500)
      .json({ message: "Server error acknowledging assignment" });
  } finally {
    connection.release();
  }
}
  ...
  try {
    ...
    if (asset.current_assignee_id !== myId) {
      return res.status(403).json({
        message:
          "You can only update the usage state of assets assigned to you",
      });
    }

    if (asset.status !== "assigned") {
      return res.status(400).json({
        message: "Usage state can only be changed while the asset is assigned",
      });
    }
    ...
    await connection.query("UPDATE assets SET usage_state = ? WHERE id = ?", [
      usage_state,
      id,
    ]);
...
export async function getAssetStats(req: Request, res: Response) {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error fetching asset stats" });
  }
}
