...
export async function updateMyProfile(req: Request, res: Response) {
  try {
    ...
    return res
      .status(500)
      .json({ message: "profile updation error on the server side" });
  }
}
...
export async function updateEmployee(req: Request, res: Response) {
  ...
  try {
    ...
    if (departmentIds !== undefined && !Array.isArray(departmentIds)) {
      return res
        .status(400)
        .json({ message: "department_ids must be an array" });
    }
    ...
    if (departmentIds !== undefined) {
      if (departmentIds.length > 0) {
        ...
        if (validDepartments.length !== new Set(departmentIds).size) {
          ...
          return res
            .status(400)
            .json({ message: "One or more department_ids do not exist" });
        }
      }
...
export async function setEmployeeActiveStatus(req: Request, res: Response) {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error changing employee status" });
  }
}
