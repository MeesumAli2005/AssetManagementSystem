...
export async function createCategory(req: Request, res: Response) {
  ...
  try {
    ...
    if (specs) {
      for (const spec of specs) {
        if (!spec.spec_name) {
          return res
            .status(400)
            .json({ message: "Spec Name is required for each spec" });
        }

        if (spec.spec_type && !VALID_SPEC_TYPES.includes(spec.spec_type)) {
          return res.status(400).json({
            message: `The spec must be of one of these types: ${VALID_SPEC_TYPES.join(", ")}`,
          });
        }
      }
    }
    ...
    return res
      .status(500)
      .json({ message: "Server side err creating the required category" });
  } finally {
    connection.release();
  }
}
  ...
  try {
    ...
    if (assetsUsingIt.length > 0) {
      return res.status(409).json({
        message: "Cannot delete a category that has assets assigned to it",
      });
    }
    const connection = await pool.getConnection();
...
export async function addSpecToCategory(req: Request, res: Response) {
  try {
    ...
    if (spec_type && !VALID_SPEC_TYPES.includes(spec_type)) {
      return res.status(400).json({
        message: `spec_type must be one of: ${VALID_SPEC_TYPES.join(", ")}`,
      });
    }
