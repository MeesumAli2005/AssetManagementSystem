...
const USERS = [
  ...
  },
  {
    ...
    email: "bilal.ahmed@eyratech.com",
  ...
  },
  {
    ...
    email: "usman.tariq@eyratech.com",
    password: "Employee@123",
    role: "employee",
    departments: ["HR"],
  },
...
const ASSETS = [
  {
    ...
    document: {
      ...
      file_url: "/uploads/seed-lap-001-receipt.pdf",
    },
  },
  {
    asset_tag: "LAP-002",
  ...
  for (const category of CATEGORIES) {
    ...
    if (existing.length > 0) {
      ...
      for (const spec of existingSpecs)
        specIds[category.name][spec.spec_name] = spec.id;
      continue;
    }
    ...
    console.log(
      `Created category "${category.name}" with ${category.specs.length} specs`,
    );
  }
...
async function seedAssets(connection, categoryIds, specIds, userIds, adminId) {
  for (const asset of ASSETS) {
    ...
    await connection.query(
      ...
      [
        adminId,
        `Asset "${asset.name}" (${asset.asset_tag}) added to inventory`,
        assetId,
      ],
    );
    ...
    if (asset.document) {
      await connection.query(
        ...
        [
          assetId,
          asset.document.document_type,
          asset.document.file_url,
          adminId,
        ],
      );
    }
