...
async function run() {
  ...
  if (rows.length > 0 && rows[0].NON_UNIQUE === 0) {
    ...
    await pool.query(
      "ALTER TABLE requests ADD INDEX resulting_asset_id_idx (resulting_asset_id)",
    );
    await pool.query("ALTER TABLE requests DROP INDEX resulting_asset_id");
    await pool.query(
      "ALTER TABLE requests RENAME INDEX resulting_asset_id_idx TO resulting_asset_id",
    );
    console.log(
      "Replaced UNIQUE index on requests.resulting_asset_id with a plain index",
    );
  } else {
    console.log(
      "requests.resulting_asset_id index is already non-unique, skipping",
    );
  }
