// combined feed of assignment receipts and return acks, sorted newest first
...
export default function Acknowledgements() {
  ...
  async function load() {
    ...
    try {
      ...
      setReturns(
        myRequests.filter(
          (r) => r.request_type === "return" && r.status === "completed",
        ),
      );
    } catch (err) {
      setError(
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <p className="mb-6 text-base text-zinc-500">
        Every asset receipt and return you've acknowledged, or still need to.
        Click one to see its full details.
      </p>
                  ...
                  >
                    <div>
                      ...
                      <p className="text-base text-zinc-500">
                        {data.category_name || "—"} · Assigned by{" "}
                        {data.assigned_by_name || "an administrator"} on{" "}
                    ...
                    <div className="flex items-center gap-3">
                      {pending ? (
                        ...
                        <StatusBadge
                          ...
                          color={data.acknowledged_at ? "green" : "red"}
                        />
                      )}
                    </div>
                  </Link>
                  ...
                  {pending ? (
                    <button
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        handleAcknowledgeReturn(request);
                      }}
                      disabled={ackingId === `return-${request.id}`}
                      className="rounded-lg bg-emerald-600 px-3.5 py-1.5 text-base font-medium text-white shadow-sm transition hover:bg-emerald-500 disabled:cursor-not-allowed disabled:opacity-50"
                    >
                      {ackingId === `return-${request.id}`
                        ? "Acknowledging…"
                        : "Acknowledge"}
                    </button>
                  ) : (
