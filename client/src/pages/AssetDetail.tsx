...
export default function AssetDetail() {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <Link
        to="/assets"
        className="mb-4 inline-block text-base text-zinc-500 hover:text-emerald-400"
      >
        ← Back to assets
      </Link>
      ...
      {!isAdmin &&
        asset.my_assignment &&
        !asset.my_assignment.acknowledged_at && (
          <div className="mb-6 flex items-center justify-between rounded-lg bg-amber-500/10 px-4 py-3">
            <p className="text-base text-amber-400">
              You haven't acknowledged receipt of this asset yet.
            </p>
            <button
      ...
      <div className="mb-6 grid grid-cols-2 gap-4 rounded-xl border border-zinc-800 bg-zinc-900/60 p-6 shadow-sm">
        ...
        {!isAdmin && asset.my_assignment && (
          <div>
            ...
            <p className="text-base text-zinc-100">
              {new Date(asset.my_assignment.assigned_at).toLocaleDateString()}
            </p>
          </div>
        )}
      </div>
      ...
      {isAdmin && (
        <>
          <div className="mb-6 rounded-xl border border-zinc-800 bg-zinc-900/60 p-6 shadow-sm">
            ...
            {asset.current_assignee_name ? (
              <p className="text-base text-zinc-400">
                Assigned to{" "}
                <Link
                  to={`/admin/employees/${asset.current_assignee_id}`}
          ...
          <div className="mb-6 rounded-xl border border-zinc-800 bg-zinc-900/60 p-6 shadow-sm">
            <p className="mb-3 text-base font-medium text-zinc-300">
              Brand, Status &amp; Condition
            </p>
            <div className="mb-4">
              ...
              <p className="mt-1.5 text-sm text-zinc-500">
                Changing this recomputes the asset's display name.
              </p>
            </div>
            <div className="mb-4 flex gap-3">
              {asset.status === "assigned" ? (
                <div className="flex flex-1 items-center gap-2 rounded-lg border border-zinc-800 bg-zinc-900 px-3 py-2 text-base text-zinc-500">
                  Status is locked to "assigned" while this asset has an
                  assignee.
                </div>
              ) : (
                <select
            ...
            {asset.status === "disposed" && asset.disposed_at && (
              <p className="mt-3 text-base text-zinc-500">
                Disposed of on{" "}
                {new Date(asset.disposed_at).toLocaleDateString()}
              </p>
            )}
          </div>
        </>
      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-6 shadow-sm">
        ...
        {!isAdmin && (
          <p className="mb-3 text-sm text-zinc-500">
            While this asset was assigned to you.
          </p>
        )}
      ...
      >
        <form onSubmit={handleDispose} className="space-y-4">
          <p className="text-base text-zinc-400">
            THIS ACTION CANNOT BE REVERSED (This is a step beyond retirement,
