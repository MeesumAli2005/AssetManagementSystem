...
export default function EmployeeDetail() {
  ...
  async function handleSave(event) {
    ...
    try {
      ...
      if (nameChanged && deptsChanged)
        message = "Profile and departments updated";
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <form
        onSubmit={handleSave}
        className="space-y-6 rounded-xl border border-zinc-800 bg-zinc-900/60 p-6 shadow-sm"
      >
        <div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {allDepartments.map((dept) => (
              <label
                key={dept.id}
                className="flex items-center gap-2 text-base text-zinc-300"
              >
                <input
                  type="checkbox"
      ...
      >
        <form onSubmit={handleResetPassword} className="space-y-4">
          <p className="text-base text-zinc-500">
            Set a temporary password for {employee.full_name}. Share it with
            them securely.
          </p>
