...
function buildTimeline(r: RequestDetailData) {
  ...
  if (r.request_type === "return" && r.acknowledged_at) {
    steps.push({
      label: "Acknowledged by employee",
...
function waitingCaption(r: RequestDetailData) {
  ...
  if (r.status === "approved" && r.request_type === "return")
    return "Awaiting return — mark completed once received";
...
export default function RequestDetail() {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <Link
        to="/admin/requests"
        className="mb-4 inline-block text-sm font-medium text-zinc-500 hover:text-zinc-300"
      >
        ← Back to requests
      </Link>
      ...
      {waitingCaption(request) && (
        <p className="mb-4 rounded-lg bg-amber-500/10 px-3 py-2 text-base text-amber-400">
          {waitingCaption(request)}
        </p>
      )}
      ...
      <div className="space-y-5 rounded-xl border border-zinc-800 bg-zinc-900/60 p-6 shadow-sm">
        ...
        {request.asset_name && (
          <Field label="Asset">
            {request.asset_name} — currently{" "}
            {request.asset_status?.replaceAll("_", " ")}
          </Field>
        )}
        ...
        {request.review_notes && (
          <Field label="Note to employee">{request.review_notes}</Field>
        )}

        {request.repair_details && (
          <Field label="Internal repair notes (private — not shown to employee)">
            {request.repair_details}
          </Field>
        )}
            ...
            {request.notes.map((n) => (
              ...
              >
                ...
                <p className="mt-1 text-sm text-zinc-500">
                  {n.admin_name} on {new Date(n.created_at).toLocaleString()} —
                  request was{' "'}
                  <span className="capitalize">
                    {n.status_at_time.replaceAll("_", " ")}
                  </span>
                  " at the time
                </p>
              </li>
      ...
      >
        <form onSubmit={handleSubmitReview} className="space-y-4">
          ...
          {reviewAction === "approved" && request.request_type === "repair" && (
            <div>
              <label className="mb-1.5 block text-base font-medium text-zinc-300">
                Internal repair notes (optional, private)
              </label>
              <textarea
                value={repairDetails}
      ...
      >
        <form onSubmit={handleAssign} className="space-y-4">
          <p className="text-base text-zinc-400">
            Fulfilling {request.employee_name}'s request for{" "}
            <span className="text-zinc-200">{request.category_name}</span>.
          </p>
          <div>
            <label className="mb-1.5 block text-base font-medium text-zinc-300">
              Available asset
            ...
            {availableAssets.length === 0 && (
              <p className="mt-1.5 text-sm text-zinc-500">
                No available assets in this category right now.
              </p>
            )}
          </div>
          <button
      ...
      >
        <form onSubmit={handleCompleteReturn} className="space-y-4">
          <p className="text-base text-zinc-400">
            ...
            <span className="text-zinc-200">{request.asset_name}</span> has been
            physically received.
          </p>
          <div>
            <label className="mb-1.5 block text-base font-medium text-zinc-300">
      ...
      <Modal
        ...
        onClose={() => setRepairCompleteOpen(false)}
        title="Mark repaired & returned"
      >
        <form onSubmit={handleCompleteRepair} className="space-y-4">
          <p className="text-base text-zinc-400">
            Confirming{" "}
            <span className="text-zinc-200">{request.asset_name}</span> has been
            repaired and returned to {request.employee_name}.
          </p>
          <div>
            <label className="mb-1.5 block text-base font-medium text-zinc-300">
              Repair completion notes (optional)
            </label>
            <textarea
              value={repairNotes}
