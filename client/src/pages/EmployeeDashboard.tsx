// employee landing page, quick stats on their own assets
      ...
      {assets && (
        <>
          <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <StatTile
              label="Under repair"
              value={assets.filter((a) => a.status === "under_repair").length}
            />
          </div>
          ...
          {assets.length > 0 && (
            <div className="mb-8">
              <BarBreakdown
                ...
                rows={CONDITION_ROWS.map((row) => ({
                  label: row.key,
                  value: byCondition[row.key],
                  color: row.color,
                }))}
              />
            </div>
